...
patients[,1]    # 네임열 (patients$name)
patients[1,]     # 1번째 인덱스 출력
...
### 결측치(NA) 처리
# 1. 결측치 확인
str(airquality) #153 obs. of  6 variables:  153data 컬럼은 6개
sum(is.na(airquality))   # 결측치 총합계 확인
# 컬럼마다 결측기 몇개 있는지 확인
for (i in 1:length(airquality)){       # 전체에서 결측치가 있는지 확인
    print(sum(is.na(airquality[,i])))   # 컬럼별로 결측치가 있는지 확인
}
